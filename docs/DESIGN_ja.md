...
| 表 / コード | 0.9F (スライドは 0.85F) / 等幅で 2 段組では 0.6F、それ以外は 0.85F | 密度の高い要素は一段小さく、本文と区別する。本文 10pt に図表 9pt という規格票の慣行と同じ比 [G3]。2 段組ではコードを 1 段に収めるので二段階小さくし、長い行は幅いっぱいの帯にせず折り返す |
...
**フォントは PDF に埋め込めるものを優先する。** 本文の日本語フォントは BIZ UDPGothic を第一候補にし、無ければ Hiragino Kaku Gothic ProN、Noto Sans JP の順で落とす。BIZ UD は Windows 10 以降に標準搭載のユニバーサルデザイン書体で、glyf 形式のため Chromium の PDF 生成がサブセット化して埋め込める。CFF 形式の Hiragino は埋め込めず、代わりに Osaka-Mono が 2.2MB 丸ごと埋め込まれる (15 ページの資料で PDF が 2.3MB になり、BIZ UD にすると 0.4MB になった)。コードとインラインコードのスタックにも BIZ UDGothic を入れ、コード中の日本語が同じ経路で肥大しないようにする。Mermaid 図の文字は本文と同じスタックを使う。BIZ UD は Regular (400) と Bold (700) の 2 ウェイトしか持たないので、見出しやスライドのタイトルに指定している 600 は 700 に解決される。太さの役割は本文との区別なので、これで足りる。既定のスタックは `--font-family` と `--code-font-family` で差し替えられ、指定はエンベロープの `layout` に記録される。

**文書は自分の字形を持つ。** 同梱の BIZ UD (SIL OFL) を文書で使う文字だけに絞ったサブセットを、`@font-face` の data URI として先頭に埋め込む。本文に Regular、見出し・表の見出し行・強調だけに Bold、コードだけに等幅の BIZ UDGothic である。計測パスも同じサブセットで行うので、ページ割りは生成側の環境にあるフォントに左右されず、BIZ UD を持たない読み手にも同じ字形で見える。PDF はシステムフォントの代わりにこのサブセットを埋め込む (15 ページの資料で HTML は 0.46MB から 0.77MB、PDF は 0.40MB から 0.24MB)。`--no-embed-fonts` で止められる。
...
**段の高さを揃えるのは帯の直前だけ。** 帯の手前では、区間のブロックを丸ごと左右に配り直して両段の下端をほぼ揃え、帯の上に穴を残さない。そのためにブロックを割ることはせず、見出しが左段の末尾に単独で残る配り方もしない。ページやセクションの末尾では揃えない。左段を上から埋め、入りきらない分だけを右段へ回すので、短いセクションは両段に薄く散らばらず上から読める。
...
**図を段に入れるか帯にするかは、どちらが大きく描けるかで決める。** 段の枠 (段幅 x 段の高さ) と帯の枠 (全幅 x 高さの 0.6) にそれぞれ収めて実測した縮小率を比べ、大きい方を選ぶ。固定の縦横比しきい値は持たない。
...
flowchart TB
    ...
    Q1 -->|2| B1["帯"]
    Q1 -->|なし| Q2{"種類"}
    Q2 -->|表 4 列以上<br/>html| B2["帯"]
    Q2 -->|段落 / リスト / コード<br/>表 3 列以下| C2["段"]
...
**縦長のフローチャートは縮めずに段組みにする。** 高さが幅の 2 倍以上ある Mermaid フローチャートが 0.9 倍未満に縮む場合、ノードの間の空いた水平位置で切り分け、1 つの SVG の中で 2 〜 4 本の帯として新聞の段のように横に並べる。段組みにするのは、図が 1.3 倍以上大きくなり、かつ文字が 11px 以上に保てるときだけ。帯の間隔は縮尺によらず図の文字サイズの 2.5 倍で、段組みにした図は高さの 0.8 まで使える。
...
- **シンタックスハイライタの同梱。** コードはビルド時に Shiki で色付けし、色は静的なマークアップとしてページに書き込む。ファイルがハイライタ本体を持つことはない。
